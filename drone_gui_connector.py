...
class DroneGUIConnector:
    ...
    def enable_proximity_sensors(self):
        ...
        try:
            ...
            if self.drone_mode == "simulation" and hasattr(self.current_drone, '_init_proximity_sensors'):
                # เรียกใช้ฟังก์ชันเริ่มต้น proximity sensors
                ...
                self.log_message("⚠️ Proximity sensors not supported")
                return False
    ...
    def get_height(self):
        ...
        try:
            if hasattr(self.current_drone, 'proximity_manager') and self.current_drone.proximity_manager:
                height = self.current_drone.proximity_manager.get_height()
                if height is not None:
                    self.log_message(f"📏 Height from ground: {height:.3f}m")
                    return height
                else:
                    self.log_message("❌ Cannot measure height")
                    return None
            elif hasattr(self.current_drone, 'get_altitude'):
                # ลองใช้ get_altitude แทน
                altitude = self.current_drone.get_altitude()
                if altitude is not None:
                    self.log_message(f"📏 Altitude: {altitude:.3f}m")
                    return altitude
                else:
                    self.log_message("❌ Cannot measure altitude")
                    return None
            else:
                self.log_message("❌ Height measurement not available")
                return None
